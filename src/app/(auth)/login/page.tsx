 ...
 const handleLogin = async (event: any) => {
  ...
  try {
   ...
   if (res?.status === 200) {
    ...
    if (res?.status === 401) {
     errorSet('Invalid email or password');
     isLoadingSet(false);
    }
   }
  } catch (error) {
  ...
  <div className='h-screen w-full flex items-center justify-center flex-col'>
   ...
   <div className='bg-white shadow-md border border-gray-200 rounded-lg w-96 p-4 sm:p-6 lg:p-8 dark:bg-gray-800 dark:border-gray-700'>
    <form className='space-y-6' onSubmit={(event) => handleLogin(event)}>
     ...
     <button
      type='submit'
      className='w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:pointer-events-none'
      disabled={isLoading}
     >
